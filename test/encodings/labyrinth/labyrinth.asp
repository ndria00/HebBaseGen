...
rrpush(T)   :- step(T), neg_goal(S), not ccpush(T).
ccpush(T)   :- step(T), neg_goal(S), not rrpush(T).
...
conn(X,Y,D,T) :- conn(XX,YY,D,S), dir(D), shift(XX,YY,X,Y,T), step(T).

%%  Location of goal after pushing

goal(X,Y,T) :- goal(XX,YY,S), shift(XX,YY,X,Y,T), step(T).

%%  Locations reachable from new position

reach(X,Y,T) :- reach(XX,YY,S), shift(XX,YY,X,Y,T), step(T).
